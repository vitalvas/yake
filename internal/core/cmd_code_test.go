...
func TestCreateGithubLangGolangCommand(t *testing.T) {
	t.Run("returns valid command", func(t *testing.T) {
		cmd := createGithubLangGolangCommand()

		require.NotNil(t, cmd)
		assert.Equal(t, "github-lang-golang", cmd.Use)
		assert.NotNil(t, cmd.RunE)
	})
	...
	t.Run("returns error when workflow already exists", func(t *testing.T) {
		...
		os.Chdir(tmpDir)
		os.MkdirAll(filepath.Join(".github", "workflows"), 0755)
		os.WriteFile(filepath.Join(".github", "workflows", "golang.yml"), []byte(""), 0644)
...
func TestCreateGoreleaserCommand(t *testing.T) {
	...
	t.Run("creates config file", func(t *testing.T) {
		...
		require.NoError(t, readErr)
		assert.True(t, strings.HasPrefix(string(content), "# generated by: yake code goreleaser\n"))
		assert.Contains(t, string(content), "testowner")
		assert.Contains(t, string(content), "testrepo")
		assert.NotContains(t, string(content), "nfpms:")
	})

	t.Run("creates config file with deb package", func(t *testing.T) {
		tmpDir := t.TempDir()
		originalDir, _ := os.Getwd()
		defer os.Chdir(originalDir)
		...
		content, readErr := os.ReadFile(".goreleaser.yml")
		require.NoError(t, readErr)
		assert.Contains(t, string(content), "nfpms:")
		assert.Contains(t, string(content), "package_name: testrepo")
		assert.Contains(t, string(content), "- deb")
	})
}
...
func TestCodeGoreleaser(t *testing.T) {
	t.Run("creates config with correct owner and repo", func(t *testing.T) {
		...
		err := codeGoreleaser(goreleaserConfig{})
		require.NoError(t, err)
		...
		assert.NotContains(t, str, "nfpms:")
	})
...
func TestCodeGithubLangGolang(t *testing.T) {
	t.Run("creates workflow file", func(t *testing.T) {
		tmpDir := t.TempDir()
		originalDir, _ := os.Getwd()
		defer os.Chdir(originalDir)
	...
	t.Run("workflow file starts with generated header", func(t *testing.T) {
		...
		content, readErr := os.ReadFile(".github/workflows/golang.yml")
		require.NoError(t, readErr)
		assert.True(t, strings.HasPrefix(string(content), "# generated by: yake code github-lang-golang\n"))
	})
